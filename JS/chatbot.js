...
const Chatbot = {
  ...
  async init() {
    ...
    // ── History Sidebar ──
    const avatarDiv = this.popup ? this.popup.querySelector(".chat-header-avatar") : null;
    ...
    if (closeHistoryBtn && this.popup) {
      closeHistoryBtn.onclick = () => this.popup.classList.remove("history-open");
    }
    ...
    if (this.popup) {
      this.container = this.popup.querySelector(".chat-container") || this.popup.querySelector(".chat-content-column");
  ...
  appendFollowUp(followUp) {
    ...
    question.innerHTML = followUp.question
      .replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>");
      ...
      ${
        result.precautions && result.precautions.length
          ? `<div class="result-precautions">
              ...
              <ol>
                ${result.precautions.filter(p => p && p.trim()).map((p) => `<li>${p}</li>`).join("")}
              </ol>
            </div>`
          : ""
      }
  ...
  _sendToBackend(message) {
    ...
    fetch(this.getBackendUrl(), {
      ...
      .then((data) => {
        ...
        if (!data.success) {
          this.appendMessage(
            data.response || "I'm sorry, something went wrong. Please try again.",
            "bot"
          );
          this._setInputVisible(true);
          return;
  ...
  resetChat() {
    ...
    // Show default welcome message immediately
    this.showWelcomeMessage();

    // Send reset to backend in the background
    fetch(this.getBackendUrl(), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  async fetchHistory() {
    ...
    try {
      ...
      data.sessions.forEach(session => {
        ...
        const dateStr = new Date(session.last_updated).toLocaleString(undefined, {
        ...
        deleteBtn.onclick = async (e) => {
          ...
          if (confirmed) {
            try {
              const deleteUrl = this.getHistoryUrl().replace("get_chat_history.php", "delete_chat_session.php");
      ...
      if (data.success && data.messages) {
        data.messages.forEach(msg => {
          ...
          if (msg.sender === "bot" && !text.includes("<img")) {
            const isInHtmlFolder = window.location.pathname.includes("/HTML/");
            const basePath = isInHtmlFolder ? "../" : "./";
            text = `<img src="${basePath}Assets/Icons/blue-chatbot.svg" alt="AI" style="width: 18px; height: 18px; vertical-align: bottom; margin-right: 6px;"> ` + text;
          }
