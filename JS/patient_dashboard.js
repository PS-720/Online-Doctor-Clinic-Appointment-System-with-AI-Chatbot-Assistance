			...
			appList.innerHTML = upcoming
                ...
                <div class="activity-item">
                    <div class="doc-name-logo">
                        ...
                        <div class="doc-name">
							<h4>Dr. ${formatName(appt.doctor_name)}</h4>
...
function renderHistoryTable(history) {
	...
	tableBody.innerHTML = history
		.map(
			(appt) => `
        <tr>
			<td style="font-weight:700;">Dr. ${formatName(appt.doctor_name)}</td>
				...
				if (!res.doctors.length) {
					...
					drSelect.innerHTML =
						...
						res.doctors
							.map(
								(d) =>
									`<option value="${d.doctor_id}">Dr. ${formatName(d.full_name)} (${d.specialization})</option>`,
							)
							.join("");
				}
			}
